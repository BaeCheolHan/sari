...
def _create_bootstrap_script(install_dir: Path):
    ...
    if IS_WINDOWS:
        ...
        content = (
            ...
            "REM Pick free ports for zero-downtime upgrades\r\n"
            "for /f \"tokens=1,2\" %%a in ('\"%~dp0\\..\\..\\..\\..\\..\\..\\..\\..\\..\\Library\\Developer\\CommandLineTools\\Library\\Frameworks\\Python3.framework\\Versions\\3.9\\Resources\\Python.app\\Contents\\MacOS\\Python\" - <<PY\r\n"
            "import socket\r\n"
            "def find_free(start, avoid=None):\r\n"
            "    avoid = set(avoid or [])\r\n"
            ...
            "    raise SystemExit(1)\r\n"
            "d = find_free(47779)\r\n"
            "h = find_free(47777, avoid=[d])\r\n"
            "print(d, h)\r\n"
            "PY') do (\r\n"
            "  set SARI_DAEMON_PORT=%%a\r\n"
            "  set SARI_HTTP_API_PORT=%%b\r\n"
            ")\r\n"
            ...
            "  exit /b 0\r\n"
            ")\r\n"
            f"\"{py}\" -m sari.mcp.cli daemon start -d --daemon-port %SARI_DAEMON_PORT% --http-port %SARI_HTTP_API_PORT%\r\n"
            f"\"{py}\" -m sari.main %*\r\n"
        )
    else:
        ...
        content = (
            ...
            "# Pick free ports for zero-downtime upgrades\n"
            "read -r SARI_DAEMON_PORT SARI_HTTP_API_PORT <<'PY'\n"
            "python3 - <<'PYC'\n"
            "import socket\n"
            "def find_free(start, avoid=None):\n"
            "    avoid = set(avoid or [])\n"
            ...
            "  exit 0\n"
            "fi\n\n"
            "python3 -m sari.mcp.cli daemon start -d --daemon-port \"$SARI_DAEMON_PORT\" --http-port \"$SARI_HTTP_API_PORT\" >/dev/null 2>&1 || true\n"
