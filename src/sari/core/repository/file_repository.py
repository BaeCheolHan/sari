...
class FileRepository(BaseRepository):
    ...
    def upsert_files_tx(self, cur: sqlite3.Cursor, rows: Iterable[tuple]) -> int:
        ...
        for r in rows:
            # Robust mapping: pad tuple if too short to match FILE_COLUMNS length
            r_list = list(r)
            ...
            # Ensure safe values for every column to prevent KeyError and IntegrityError
            row_dict = {
                "path": str(path),
        ...
        if not processed_rows: return 0

        col_names = ", ".join(FILE_COLUMNS)
        placeholders = ", ".join(["?"] * len(FILE_COLUMNS))
        update_set = ", ".join([f"{col}=excluded.{col}" for col in FILE_COLUMNS if col != "path"])
        ...
        cur.executemany("DELETE FROM symbols WHERE path = ?", [(r[0],) for r in processed_rows])
    ...
    def get_file_meta(self, path: str) -> Optional[Tuple[int, int, str]]:
        ...
        try:
            ...
            ch = json.loads(row["metadata_json"]).get("content_hash", "") if row["metadata_json"] else ""
    ...
    def list_files(self, limit: int = 50, repo: Optional[str] = None, root_ids: Optional[List[str]] = None) -> List[Dict]:
        ...
        return [{"path": r["path"], "size": r["size"], "repo": r["repo"]} for r in self.execute(sql, params).fetchall()]
    ...
    def get_repo_stats(self, root_ids: Optional[List[str]] = None) -> Dict[str, int]:
        ...
        if root_ids: sql += f" AND root_id IN ({','.join(['?']*len(root_ids))})"; params.extend(root_ids)
        sql += " GROUP BY repo"
        return {r["repo"]: r["c"] for r in self.execute(sql, params).fetchall()}
