...
class ServerRegistry:
    ...
    def get_active_daemons(self) -> list[Dict[str, Any]]:
        """Return all daemons that are currently alive."""
        data = self._load()
        daemons = []
        for bid, info in data.get("daemons", {}).items():
            if self._is_process_alive(info.get("pid")):
                d = dict(info)
                d["boot_id"] = bid
                daemons.append(d)
    ...
    def resolve_latest_daemon(self, workspace_root: Optional[str] = None, allow_draining: bool = False) -> Optional[Dict[str, Any]]:
        """
        SSOT for picking the 'best' daemon.
        Priority:
          1. Non-draining daemons (unless allow_draining is True)
          2. Latest version string
          3. Newest start_ts
        """
        daemons = self.get_active_daemons()
        if not daemons:
            return None

        # Filter out draining daemons if not allowed
        if not allow_draining:
            daemons = [d for d in daemons if not d.get("draining")]
        
        if not daemons:
            return None

        # Sort by version (desc) and start_ts (desc)
        # Note: In real scenarios, version comparison should be more robust
        daemons.sort(key=lambda x: (x.get("version", ""), x.get("start_ts", 0)), reverse=True)
        
        # If workspace_root is provided, check if any of these daemons are already serving it
        if workspace_root:
            ws = self._normalize_workspace_root(workspace_root)
            data = self._load()
            ws_info = data.get("workspaces", {}).get(ws)
            if ws_info:
                current_bid = ws_info.get("boot_id")
                # If the daemon currently bound to the workspace is in our top list, use it
                for d in daemons:
                    if d["boot_id"] == current_bid:
                        return d

        return daemons[0]

    def set_workspace_http(self, workspace_root: str, http_port: int, http_host: Optional[str] = None, http_pid: Optional[int] = None, boot_id: Optional[str] = None) -> None:
        def _upd(data):
            ...
            if boot_id: payload["boot_id"] = str(boot_id)
    ...
    def touch_daemon(self, boot_id: str) -> None:
        """Update last_seen_ts for a daemon to indicate it's alive."""
        def _upd(data):
            daemons = data.get("daemons", {})
