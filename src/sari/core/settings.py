...
class Settings(BaseSettings):
    if HAS_PYDANTIC:
        model_config = SettingsConfigDict(env_prefix="SARI_", case_sensitive=True, extra="ignore")
    ...
    MAX_PARSE_BYTES: int = 1024 * 1024 # 1MB (Added)
    MAX_AST_BYTES: int = 512 * 1024 # 512KB (Added)
    FTS_MAX_BYTES: int = 1000000
    ENGINE_MAX_DOC_BYTES: int = 50000
    ...
    DAEMON_AUTOSTART: bool = True # Default to True for autostart policy
    ...
    MMAP_SIZE: int = 30 * 1024 * 1024 * 1024 
    PAGE_SIZE: int = 65536 
    MAX_DEPTH: int = 20
    ...
    def get_int(self, key: str, default: int) -> int:
        ...
        return int(val) if val is not None else int(os.environ.get(f"SARI_{key}", default))
    
    def get_bool(self, key: str, default: bool) -> bool:
        val = getattr(self, key, None)
        if val is not None: return bool(val)
        return os.environ.get(f"SARI_{key}", str(default)).lower() in ("1", "true", "yes", "on")
