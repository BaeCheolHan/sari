...
class WorkspaceManager:
    ...
    def root_id(path: str) -> str:
        """Stable root id derived from project root boundary (Absolute Path)."""
        return WorkspaceManager.normalize_path(WorkspaceManager.find_project_root(path))

    @staticmethod
    def root_id_for_workspace(workspace_root: str) -> str:
        """Stable root id for an explicit workspace root (Absolute Path)."""
    ...
    def get_engine_index_dir(policy: Optional[str] = None, roots: Optional[List[str]] = None, root_id: Optional[str] = None) -> Path:
        ...
        if policy in {"per_root", "shard"}:
            ...
            # Priority Fix: Use path-based naming instead of hashing
            if os.sep in rid or (os.altsep and os.altsep in rid):
                # Clean path for directory naming (remove leading slash, replace others)
                sanitized = rid.strip(os.sep).replace(os.sep, "_").replace(":", "_")
                # Keep it reasonably short but identifiable
