...
class LocalSearchMCPServer:
    ...
    def __init__(self, workspace_root: str, cfg: Any = None, db: Any = None, indexer: Any = None, start_worker: bool = True):
        ...
        self._debug_enabled = settings.DEBUG or os.environ.get("SARI_MCP_DEBUG", "0") == "1" or os.environ.get("SARI_MCP_DEBUG_LOG", "0") == "1"
        if os.environ.get("SARI_MCP_DEBUG_LOG", "0") == "1":
            log_dir = WorkspaceManager.get_global_data_dir()
            log_dir.mkdir(parents=True, exist_ok=True)
            log_file = log_dir / "mcp_debug.log"
            import logging as std_logging
            h = std_logging.FileHandler(str(log_file), encoding="utf-8")
            h.setFormatter(std_logging.Formatter("%(message)s"))
            std_logger = std_logging.getLogger("sari.mcp.protocol")
            std_logger.addHandler(h)
            std_logger.setLevel(std_logging.DEBUG)
            # Ensure processors are applied if possible, or just rely on std_logger
    ...
    def run(self) -> None:
        ...
        if not self.transport:
            ...
            # Priority 10 Fix: Force Content-Length framing for better reliability with standard MCP clients
