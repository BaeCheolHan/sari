...
class ClangdLanguageServer(SolidLanguageServer):
    ...
    def _start_server(self) -> None:
        ...
        def lang_status_handler(params: dict[str, object]) -> None:
            # 참고: Should we wait for
            # server -> client: {'jsonrpc': '2.0', 'method': 'language/status', 'params': {'type': 'ProjectStatus', 'message': 'OK'}}
            # Before proceeding?
            if params["type"] == "ServiceReady" and params["message"] == "ServiceReady":
                self.service_ready_event.set()

        def execute_client_command_handler(params: dict[str, object]) -> list[object]:
            return []

        def do_nothing(params: dict[str, object]) -> None:
            return

        def check_experimental_status(params: dict[str, object]) -> None:
