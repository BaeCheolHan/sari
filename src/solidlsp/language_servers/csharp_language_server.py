...
from ._adapter_common import ensure_paths_exist
...
def breadth_first_file_scan(root_dir: str) -> Iterable[str]:
    ...
    while queue:
        ...
        try:
            ...
            log.debug("Skipping unreadable directory during breadth-first scan: %s (%s)", current_dir, exc)
...
class CSharpLanguageServer(SolidLanguageServer):
    ...
    class DependencyProvider(LanguageServerDependencyProvider):
        ...
        def _ensure_server_installed(self) -> tuple[str, str]:
            ...
            """
            runtime_dependency_overrides = cast(list[dict[str, object]], self._custom_settings.get("runtime_dependencies", []))
        ...
        def _ensure_dotnet_runtime(self) -> str:
            ...
            if system_dotnet:
                try:
                    ...
                    log.debug("Failed to inspect system dotnet runtime: %s", exc)
        ...
        def _ensure_language_server(self, lang_server_dep: RuntimeDependency) -> str:
            ...
            try:
                ensure_paths_exist([str(server_dll)], context="csharp_language_server")
            except RuntimeError as exc:
                raise SolidLSPException("Roslyn Language Server DLL not found after extraction") from exc
