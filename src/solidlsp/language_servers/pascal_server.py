...
class PascalLanguageServer(SolidLanguageServer):
    ...
    def _mark_for_cleanup(cls, pasls_dir: str, file_path: str) -> None:
        ...
        try:
            ...
            log.warning(f"Failed to mark file for cleanup '{file_path}': {exc}")
    ...
    def _cleanup_old_files(cls, pasls_dir: str) -> None:
        ...
        try:
            ...
            log.warning(f"Failed to cleanup old pasls files: {exc}")
        ...
        try:
                ...
                if expected_sha256:
                    ...
                    if not cls._verify_checksum(archive_path, expected_sha256):
                        ...
                        try:
                            ...
                            log.warning(f"Failed to remove invalid archive '{archive_path}': {exc}")
                        return False
            ...
            if os.path.exists(binary_path):
                try:
                    ...
                    log.debug(f"chmod skipped for '{binary_path}': {exc}")  # May fail on Windows
            ...
            try:
                ...
                log.debug(f"Archive cleanup skipped: {exc}")
            ...
            if os.path.exists(backup_dir) and not os.path.exists(pasls_dir):
                ...
                except (OSError, RuntimeError, ValueError, TypeError) as rollback_error:
            ...
            if os.path.exists(temp_dir):
                try:
                    ...
                    log.warning(f"Failed to cleanup temp directory '{temp_dir}': {exc}")
    ...
    def _setup_runtime_dependencies(cls, solidlsp_settings: SolidLSPSettings) -> str:
        ...
        ensure_paths_exist([pasls_executable_path], context="pascal_language_server")

        try:
            os.chmod(pasls_executable_path, 0o755)
        except OSError as exc:
            log.debug(f"chmod skipped for '{pasls_executable_path}': {exc}")  # May fail on Windows, ignore
