...
class SolidLanguageServerHandler:
    ...
    def __init__(
        ...
        self.request_id = 1
        self._pending_requests: dict[RequestId, Request] = {}
        self.on_request_handlers: dict[str, RequestHandler] = {}
        self.on_notification_handlers: dict[str, NotificationHandler] = {}
        self.logger = logger
    ...
    def _safely_close_pipe(self, pipe: object | None) -> None:
        ...
        if pipe:
            try:
                ...
                log.debug("Failed to close language server pipe cleanly: %s", exc)
    ...
    def _signal_process_tree(self, process: subprocess.Popen[bytes], terminate: bool = True) -> None:
        ...
        try:
            ...
            log.debug("Failed to inspect language server parent process(pid=%s): %s", process.pid, exc)
        ...
        if parent and parent.is_running():
            ...
            for child in parent.children(recursive=True):
                try:
                    ...
                    log.debug("Failed to %s child process(pid=%s): %s", signal_method, child.pid, exc)

            # Then signal the parent
            try:
                getattr(parent, signal_method)()
            except (psutil.NoSuchProcess, psutil.AccessDenied, psutil.ZombieProcess, OSError) as exc:
                log.debug("Failed to %s parent process(pid=%s): %s", signal_method, parent.pid, exc)
        else:
            # Fall back to direct process signaling
            try:
                getattr(process, signal_method)()
            except (ProcessLookupError, OSError, RuntimeError, ValueError) as exc:
                log.debug("Failed to %s process(pid=%s): %s", signal_method, process.pid, exc)
            ...
            if isinstance(decoded_payload, list):
                for item in decoded_payload:
                    if isinstance(item, dict):
                        self._receive_payload(item)
                    else:
                        self._log(f"Unknown batch payload entry type: {item}")
                return
            if isinstance(decoded_payload, dict):
                self._receive_payload(decoded_payload)
                return
            self._log(f"Unknown payload type: {decoded_payload}")
    ...
    def send_response(self, request_id: RequestId, params: PayloadLike) -> None:
        """
    ...
    def send_error_response(self, request_id: RequestId, err: LSPError) -> None:
        """
    ...
    def send_batch_requests(self, requests: list[tuple[str, dict | None]]) -> list[PayloadLike]:
        """
        여러 요청을 JSON-RPC batch 배열로 한 번에 전송하고 순서대로 결과를 반환한다.
        """
        if len(requests) == 0:
            return []

        request_items: list[tuple[int, str, Request]] = []
        payloads: list[StringDict] = []
        with self._request_id_lock:
            for method, params in requests:
                request_id = self.request_id
                self.request_id += 1
                request = Request(request_id=request_id, method=method)
                request_items.append((request_id, method, request))
                payloads.append(make_request(method, request_id, params))
    ...
    def on_notification(self, method: str, cb: NotificationHandler) -> None:
        """
    ...
    def _request_handler(self, response: StringDict) -> None:
        ...
        except (TypeError, ValueError, RuntimeError, SolidLSPException) as ex:
