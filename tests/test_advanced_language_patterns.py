...
'''
        ...
        controller = next(s for s in symbols if s[3] == "UserController")
        meta = json.loads(controller[9])
...
'''
        ...
        meta = json.loads(service[9])
        assert "Service" in meta["annotations"]
        
        create_method = next(s for s in symbols if s[3] == "createOrder")
        assert "Transactional" in json.loads(create_method[9])["annotations"]
...
'''
        ...
        component = next(s for s in symbols if s[3] == "UserProfile")
        assert component[4] == "class"  # React 컴포넌트는 class로 분류
...
'''
        ...
        # 라우트 핸들러 감지
        route_symbols = [s for s in symbols if s[3].startswith("route.")]
