...
def test_worker_file_disappeared_mid_process():
    ...
    res = worker.process_file_task(root, file_path, mock_st, int(time.time()), time.time(), False)
    assert res is None
...
def test_db_writer_resilience_to_batch_failure():
    ...
    tasks = [
        DbTask(kind="upsert_files", rows=[("p1", "r1", "id", "repo", 0, 0, "", "", "", 0, 0, "ok", "", "ok", "", 0, 0, 0, 0, "{}")]),
        DbTask(kind="upsert_files", rows=[("p2", "r2", "id", "repo", 0, 0, "", "", "", 0, 0, "ok", "", "ok", "", 0, 0, 0, 0, "{}")])
    ]
...
async def test_daemon_duplicate_endpoint_prevention_deep():
    ...
    mock_registry.resolve_daemon_by_endpoint.return_value = {"pid": 1, "host": host, "port": port}
