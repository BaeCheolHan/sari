...
def _prepare_db_v2(conn):
    ...
    try:
        conn.execute("ALTER TABLE symbols ADD COLUMN importance_score REAL DEFAULT 0.0")
...
def test_intelligence_fuzzy_autorun(db, workspace):
    ...
    res = IndexingResult(path=f"{rid}/user.py", rel="user.py", root_id=rid, repo="repo", type="new")
    ...
    conn.execute("INSERT INTO symbols (symbol_id, path, root_id, name, kind, line, end_line, content, qualname) VALUES (?,?,?,?,?,?,?,?,?)",
                 ("s1", f"{rid}/user.py", rid, "UserService", "class", 1, 10, "", "UserService"))
...
def test_intelligence_centrality_ranking(db, workspace):
    ...
    # File A: 중요도 높음
    row_a = IndexingResult(path=f"{rid}/engine.py", rel="engine.py", root_id=rid, repo="repo", type="new", fts_content="core engine logic").to_file_row()
    # File B: 중요도 낮음
    row_b = IndexingResult(path=f"{rid}/util.py", rel="util.py", root_id=rid, repo="repo", type="new", fts_content="core utility helper").to_file_row()
    
    db.upsert_files_turbo([row_a, row_b])
    db.finalize_turbo_batch()
    
    # 심볼 중요도 강제 설정
    conn.execute("INSERT INTO symbols (symbol_id, path, root_id, name, kind, line, end_line, content, qualname, importance_score) VALUES (?,?,?,?,?,?,?,?,?,?)",
                 ("s-eng", f"{rid}/engine.py", rid, "MainEngine", "class", 1, 10, "", "MainEngine", 100.0))
    conn.execute("INSERT INTO symbols (symbol_id, path, root_id, name, kind, line, end_line, content, qualname, importance_score) VALUES (?,?,?,?,?,?,?,?,?,?)",
                 ("s-util", f"{rid}/util.py", rid, "Helper", "func", 1, 10, "", "Helper", 1.0))
...
def test_intelligence_semantic_similarity(db, workspace):
    ...
    conn.execute("INSERT INTO embeddings (root_id, entity_type, entity_id, content_hash, model, vector, created_ts, updated_ts) VALUES (?,?,?,?,?,?,?,?)",
                 (rid, "file", f"{rid}/auth.py", "h1", "text-embedding-3", vec, 0, 0))
    
    query_vector = [0.9, 0.1, 0.0]
    hits = db.search_repo.search_semantic(query_vector, limit=5, root_ids=[rid])
