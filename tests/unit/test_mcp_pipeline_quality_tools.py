...
def test_mcp_pipeline_quality_run_and_report(tmp_path: Path) -> None:
    """pipeline_quality_run/report 도구가 응답을 반환해야 한다."""
    ...
    benchmark_response = server.handle_request(
        {
            ...
            "params": {
                ...
                "arguments": {"repo": str(repo_dir.resolve()), "target_files": 10, "profile": "default"},
            },
        }
    )
    assert benchmark_response.to_dict()["result"]["isError"] is False
