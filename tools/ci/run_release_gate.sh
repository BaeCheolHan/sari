...
def read_by_id(fd, request_id: int, timeout_sec: float) -> dict[str, object]:
    end = time.time() + timeout_sec
    while time.time() < end:
        item = read_one(fd, timeout_sec=max(0.1, end - time.time()))
...
proc.stdin.flush()
resp2 = read_by_id(proc.stdout, request_id=2, timeout_sec=10.0)
if resp2.get('id') != 2:
